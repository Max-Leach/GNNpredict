...
class EdgeNeighborUpdate(nn.Module):
    ...
    '''
    def __init__(self, in_feat_sizes, out_size, inner_layer_sizes=[], residual=False, bias=False, dropout=0.0, activation_fn=nn.ReLU):
        super().__init__()

        self.residual = residual
        in_mlp_size = sum(in_feat_sizes.values())
        self.fc = mlp_from_sizes(in_mlp_size, out_size, inner_layer_sizes, bias=bias, batch_norm=True, dropout=dropout, activation_fn=activation_fn)
...
class AtomAggregUpdate(nn.Module):
    ...
    '''
    def __init__(self, atom_edge_feat_aggreg, in_feat_sizes, out_size, inner_layer_sizes=[], residual=False, bias=False, include_edges=True, dropout=0.0, activation_fn=nn.ReLU):
        super().__init__()
...
class GlobalAggregUpdate(nn.Module):
    def __init__(self, edge_aggreg, atom_aggreg, in_feat_sizes, out_size, inner_layer_sizes=[], residual=False, bias=False, dropout=0.0, activation_fn=nn.ReLU):
        super().__init__()
