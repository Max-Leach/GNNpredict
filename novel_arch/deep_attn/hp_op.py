...
def eval_on_config(config, train_loader, valid_tester, train_set):
    ...
    model = DeepAttn(
                ...
                graph_inner_layer_sizes=[[64]] * config['graph_layer_count'],
                residual=True
            )
    loss_fn = MSELoss()
...
def get_dataset():
    dset = bdedataset_from_csv('/home/pmistry/Documents/research/data/ALFABET_data/acp_updated_NoDupes.csv', max_lines=200, start_line=1)
...
def main():
    train_loader, valid_tester, train_set = get_dataset()

    config = {
        "graph_hidden_size": tune.choice([2**i for i in range(6)]),
        "graph_layer_count": tune.choice([i for i in range(5)]),
        "lr": tune.loguniform(1e-4, 1e-1),
        # "epochs": tune.choice([10, 20, 25, 30]),
    ...
    result = tune.run(
                lambda config: eval_on_config(config, train_loader, valid_tester, train_set), 
                config=config, 
                num_samples=10, 
                scheduler=scheduler
                )
