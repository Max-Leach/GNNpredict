...
def sum_injective_batch(args):
    ...
    kfold = KFoldCV(10, lambda: BDEDataset.load(dset_path),
    ...
    kfold.run_on_model_construct(model_construct, lambda lr_sched: lambda scores, epoch: lr_sched.step(scores['loss']), lr_sched_construct=lambda op: ReduceLROnPlateau(op, factor=0.8, patience=15, threshold=1e-2), path=dump_path)