...
4. **Train model given hyperparameters**
    ...
    Train model given a set of hyperparameters. We show the hyperparameters used in our final model here. The `dset_path` should point to the `dset/` dir when the encoded dataset is generated. `training_indices_path` and `valid_indices_path` should point to subset index files generated by above.
    
    This code supports training restart - if the path has a pre-existing train save and remaining arguments to the function are the same, training will resume from the last epoch recorded.
